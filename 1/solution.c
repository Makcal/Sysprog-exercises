...
struct coro_context {
    ...
    /* Start_time is initialized where a coroutine wakes up, end_time - before
     * each yield */
...
static void coro_start_timer(coro_context *context) {
    clock_gettime(CLOCK_MONOTONIC, &context->start_time);
}

static void coro_end_timer(coro_context *context) {
...
static void pool_append_file(char *name, files_pool *pool) {
...
static char *poll_next_file(files_pool *pool) {
    if (pool->size == 0)
        return NULL;
...
static void *realloc_array(void *array, size_t new_capacity) {
    void *updated_array = realloc(array, new_capacity * sizeof(void *));
    assert(updated_array != NULL);
    return updated_array;
}

static file_content *read_file(char *name) {
...
static void write_file(file_content *content, char *file_name) {
...
static void merge_sort(file_content *content, coro_context *casted_context) {
...
static void merge_all_files(files_pool *pool, char *merged_file_name) {
    files_list *iterator = new_iterator(pool);
    ...
    file_content merged_content = {.array = merged, .size = total_size};
